...
class QcProcessModelGenerator < ActiveRecord::Generators::ModelGenerator
  ...
  def create_migration_file
    ...
    migration_template "../../migration/templates/create_table_migration.rb", File.join("db_qc_process/migrate", "create_#{table_name}.rb")
    ...
    migrate_full_path = Dir[Rails.root.join("db_qc_process/migrate") + "*create_#{table_name}.rb"][0]
    ...
    unless migrate_full_path.nil?
      ...
      if text =~ /ActiveRecord::Migration\[(.*?)\]/
        new_class = "QcProcessMigrationBase[#{$1}]"
      end
    ...
    unless model_full_path.nil? or not File.exists?(model_full_path)
      ...
      new_contents = text.gsub(/ApplicationRecord/, "QcProcessSchemaBase")
