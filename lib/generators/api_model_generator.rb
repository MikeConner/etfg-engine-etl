...
class FeedModelGenerator < ActiveRecord::Generators::ModelGenerator
  ...
  def create_migration_file
    ...
    migration_template "../../migration/templates/create_table_migration.rb", File.join("db_api/migrate", "create_#{table_name}.rb")
    ...
    migrate_full_path = Dir[Rails.root.join("db_api/migrate") + "*create_#{table_name}.rb"][0]
    ...
    unless model_full_path.nil? or not File.exists?(model_full_path)
      ...
      new_contents = text.gsub(/ApplicationRecord/, "ApiSchemaBase")
